...
class TemporalMAE(nn.Module):
    ...
    def forward(self, flood_img, pre_event_1, pre_event_2):
        ...
        # Random masking across all temporal patches
        num_masked = int(self.masking_ratio * total_patches)
        rand_indices = torch.rand(batch, total_patches, device=device).argsort(dim=-1)
